...
negatives <- vals < 0 # create a logical vector indicating which elements are negative
negatives[1:5]        
vals[negatives]       # these are the values that are negative
vals[negatives] <- 0  # they are each replaced with zero
...
## 3 ##
curdir <- getwd()
## Next we'll copy the file to a different directory (you'll probably do this via the File Manager on your computer or some other approach)
...
gm_by_year <- split(gapminder$pop, gapminder$year)
pop <- data.frame(year = names(gm_by_year),
                  globalPop = sapply(gm_by_year, sum))

# using aggregate()
pop <- aggregate(gapminder$pop, by = list(year = gapminder$year),
                 FUN = sum)
names(pop)[2] <- 'globalPop'

## 8 ##
gapminder2 <- merge(gapminder, pop, by.x = 'year',
                    by.y = 'year', all.x = TRUE, all.y = FALSE)
plot(gapminder2$globalPop, gapminder2$lifeExp)
...
# This should be faster -- no need to sort the entire data frame, just find the minimum.
